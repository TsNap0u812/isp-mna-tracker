...
npm run refresh-data # scripts/refresh-data.mjs (standalone CLI fetch)
```
...
│
├── Data
│   ├── src/data/deals.json     — 49 static deals (pure JSON, database-ready)
...
│   ├── data/
│   │   └── deals.json          49 static deals (pure JSON array, database-ready)
│   └── hooks/
│       └── useLocalDeals.js    localStorage CRUD for user-added deals
├── scripts/
│   └── refresh-data.mjs        Standalone CLI version of the refresh fetch
├── package.json
├── vite.config.js
├── tailwind.config.js
├── WIKI.md                     ← this file
